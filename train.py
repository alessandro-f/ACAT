...
class CustomImageDataset(Dataset):
    def __init__(self, images,labels, process, transform, target_transform=None):
...
train_dataset = CustomImageDataset(images = X_train, labels = y_train, process = process_scan, transform = transform_flip)
val_dataset = CustomImageDataset(images = X_val, labels = y_val, process = process_scan, transform = transform_noflip)
test_dataset = CustomImageDataset(images = X_test, labels = y_test, process = process_scan, transform = transform_noflip)
...
if args.model =='baseline':
    model = resnet50_baseline(block=Bottleneck,
                              layers=[3, 4, 6, 3])
    best_model = resnet50_baseline(block=Bottleneck,
                                   layers=[3, 4, 6, 3])
elif args.model =='image attention':
    model = resnet50_image_attention(block=Bottleneck,
                              layers=[3, 4, 6, 3])
    best_model = resnet50_image_attention(block=Bottleneck,
                                   layers=[3, 4, 6, 3])

else:
    print('use script: train_two_branches.py')
...
with torch.no_grad():
    ...
    best_model_dummy_out = best_model.forward(dummy_inputs)
...
def eval_iter(metric_tracker, model, x, y, ids, iteration, epoch, set_name, final_pred, final_label):
...
if __name__ == "__main__":
    ...
    _ = restore_model(restore_fields={"model": model}, path=saved_models_filepath, device=device, best=True)
